...
class IAMPolicySimulatorValidator(PolicyFilterInterface):
    ...
    def _validate_glue_policies(self, permissionsList : PermissionsList) -> PermissionsList:
        ...
        #TODO: Pass glue_resource_policy into simulator when supported
        glue_resource_policy = None

        # Split by resource type because simulate_principal_policy evaluates
        # every action × resource combination. Mixing resource types would cause
        # false positives — e.g. glue:GetTable on a database ARN could return
        # implicitDeny and incorrectly filter out a valid permission.
    ...
    def _validate_permissions(self, permissionsList : PermissionsList, resource_policy: str) -> int:
        '''Validates permissions per principal using IAM Policy Simulator.
        Batches resources to stay under the 1000 action×resource combination limit.
        '''
        ...
        for principal_arn in permissionsList.get_principal_arns():
            ...
            # Split resources into pages to stay under the 1000 combination limit
            max_resources_per_page = max(1, 1000 // len(action_names))
            resource_arns_pages = [
                resource_arns[i:i + max_resources_per_page]
                for i in range(0, len(resource_arns), max_resources_per_page)
            ]

            logger.debug(f"Principal {principal_arn}: {len(action_names)} actions, "
                         f"{len(resource_arns)} resources, {len(resource_arns_pages)} API pages")

            paginator = self._iam_client.get_paginator('simulate_principal_policy')
            for resource_arns_page in resource_arns_pages:
                if not resource_arns_page:
                    continue
                logger.debug(f"Submitting {len(resource_arns_page)} resource_arns out of {len(resource_arns)}.")
    ...
    def _inspect_simulation_result(self, result, principal_arn, permsForPrincipal) -> None:
        if 'EvalDecision' not in result:
            logger.error(f"EvalDecision is Null! {result}.")
        elif result['EvalDecision'] != 'allowed':
            ...
            if 'ResourceSpecificResults' in result:
                for resource_decision in result['ResourceSpecificResults']:
                    if resource_decision['EvalResourceDecision'] != 'allowed':
                        logger.debug(f"Permission not allowed. Not adding to list. Principal: {principal_arn}, Action: {result['EvalActionName']}, Resource: {result['EvalResourceName']}")
                        self._add_filtered_permission(principal_arn, resource_decision["EvalResourceName"], result['EvalActionName'])
            else:
                logger.debug(f"Permission not allowed as there is no ResourceSpecificResults. Removing from list. Principal: {principal_arn}, Action: {result['EvalActionName']}, Resource: {result['EvalResourceName']}")
